...
- (id)initWithStyle:(UITableViewStyle)style initialSelectedMoleculeIndex:(NSInteger)initialSelectedMoleculeIndex;
{
	...
	{        
        self.title = NSLocalizedStringFromTable(@"Molécules", @"Localized", nil);
...
//			tableTextColor = [[UIColor blackColor] retain];
			UIBarButtonItem *modelButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedStringFromTable(@"Modèle 3D", @"Localized", nil) style:UIBarButtonItemStylePlain target:self action:@selector(switchBackToGLView)];
...
- (void)moleculeDidFinishDownloading:(NSNotification *)note;
{
	...
	if (newMolecule == nil)
	{
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"Erreur durant le téléchargement du fichier", @"Localized", nil) message:NSLocalizedStringFromTable(@"Le fichier du molécule est corrompu ou n'est pas d'un format supporté par l'application", @"Localized", nil)
		...
		{
			UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"Impossible de supprimer le fichier", @"Localized", nil) message:[error localizedDescription]
														   delegate:self cancelButtonTitle:NSLocalizedStringFromTable(@"OK", @"Localized", nil) otherButtonTitles: nil, nil];
			[alert show];
			return;
		}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
	...
	if (index == 0)
	{
		cell = [tableView dequeueReusableCellWithIdentifier:@"Télécharger"];
		if (cell == nil) 
		{
			cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"Télécharger"];
		...
		cell.textLabel.text = NSLocalizedStringFromTable(@"Télécharger un nouveau molécule", @"Localized", nil);
        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
		cell.textLabel.textColor = [UIColor blackColor];
	}
	else
	{
		cell = [tableView dequeueReusableCellWithIdentifier:@"Molécules"];
		if (cell == nil) 
		{
			cell = [[SLSMoleculeLibraryTableCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"Molécules"];

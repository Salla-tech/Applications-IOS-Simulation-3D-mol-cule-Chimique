...
- (id)initWithStyle:(UITableViewStyle)style 
{
	if ((self = [super initWithStyle:style])) 
	{
		...
		self.navigationItem.title = NSLocalizedStringFromTable(@"Source de données en ligne", @"Localized", nil);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath 
{
	...
	UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"Télécharger"];
	if (cell == nil) 
	{
//		cell = [[[UITableViewCell alloc] initWithFrame:CGRectZero reuseIdentifier:@"Download"] autorelease];
	...
	switch (index)
	{
		case 0: cell.textLabel.text = NSLocalizedStringFromTable(@"RCSB Banque de données de protéines", @"Localized", nil); break;
		case 1: cell.textLabel.text = NSLocalizedStringFromTable(@"Personnaliser la location", @"Localized", nil); break;
	}

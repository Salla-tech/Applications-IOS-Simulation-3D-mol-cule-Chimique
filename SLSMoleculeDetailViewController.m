...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section 
{
    switch (section) 
	{
        case DESCRIPTION_SECTION:
            return NSLocalizedStringFromTable(@"Déscription", @"Localized", nil);
        case STATISTICS_SECTION:
            return NSLocalizedStringFromTable(@"Statistique", @"Localized", nil);
        ...
        case AUTHOR_SECTION:
            return NSLocalizedStringFromTable(@"Auteur", @"Localized", nil);
        case SEQUENCE_SECTION:
            return NSLocalizedStringFromTable(@"Séquence", @"Localized", nil);
		default:
			break;
    }
    return nil;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.section == STATISTICS_SECTION) 
	{
		...
		switch (indexPath.row)
		{
			case 0:
			{
				cell.textLabel.text = NSLocalizedStringFromTable(@"Nom du fichier", @"Localized", nil);
				cell.detailTextLabel.text = molecule.filename;
			}; break;
			case 1:
			{
				cell.textLabel.text = NSLocalizedStringFromTable(@"Nombre d'atomes", @"Localized", nil);
				cell.detailTextLabel.text = [NSString stringWithFormat:@"%d", molecule.numberOfAtoms];
			}; break;
			case 2:
			{
				cell.textLabel.text =NSLocalizedStringFromTable(@"Nombre de structures", @"Localized", nil);
				cell.detailTextLabel.text = [NSString stringWithFormat:@"%d", molecule.numberOfStructures];
			}; break;
			case 3:
			{
				cell.textLabel.text = NSLocalizedStringFromTable(@"Structure courent", @"Localized", nil);

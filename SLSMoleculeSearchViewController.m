...
- (id)initWithStyle:(UITableViewStyle)style 
{
	if ((self = [super initWithStyle:style])) 
	{
		...
		keywordSearchBar.placeholder = NSLocalizedStringFromTable(@"Rechercher PubChem", @"Localized", nil);
		keywordSearchBar.delegate = self;
		keywordSearchBar.autocorrectionType = UITextAutocorrectionTypeNo;
        keywordSearchBar.scopeButtonTitles = [NSArray arrayWithObjects:@"PubChem", @"Protéines", nil];
...
//        }
		...
		self.navigationItem.title = NSLocalizedStringFromTable(@"Rechercher un Molécules", @"Localized", nil);
...
{	
	...
	{
		cell = [tableView dequeueReusableCellWithIdentifier:@"Recherche en cours"];
		if (cell == nil) 
		{		
//			cell = [[[UITableViewCell alloc] initWithFrame:CGRectZero reuseIdentifier:@"SearchInProgress"] autorelease];
            cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"Recherche en cours"];
...
//			CGRect frame = CGRectMake(CGRectGetMaxX(cell.contentView.bounds) - 70.0, 14.0, 32.0, 32.0);
		...
		cell.textLabel.text = NSLocalizedStringFromTable(@"Recherche...", @"Localized", nil);
	}
	else if (searchResultTitles == nil)
    {
		cell = nil;
	...
	{
		cell = [tableView dequeueReusableCellWithIdentifier:@"Aucun résultat"];
		if (cell == nil) 
		{		
//			cell = [[[UITableViewCell alloc] initWithFrame:CGRectZero reuseIdentifier:@"NoResults"] autorelease];
            cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"Aucun résultat"];
...
//            }
			...
			cell.textLabel.font = [UIFont systemFontOfSize:16.0];
			cell.textLabel.text = NSLocalizedStringFromTable(@"Aucun résultat", @"Localized", nil);
			cell.textLabel.textAlignment = UITextAlignmentCenter;
			cell.accessoryType = UITableViewCellAccessoryNone;
		}
	}
    ...
    {
        cell = [tableView dequeueReusableCellWithIdentifier:@"Téléchargement en cours"];
		if (cell == nil) 
		{		
            
//            cell = [[[UITableViewCell alloc] initWithFrame:CGRectZero reuseIdentifier:@"DownloadInProgress"] autorelease];
            cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"Téléchargement en cours"];
...
//            }
        ...
        cell.textLabel.text = NSLocalizedStringFromTable(@"Téléchargement...", @"Localized", nil);
    }
	else
	{
		if ([indexPath row] >= [searchResultTitles count])
		{
			cell = [tableView dequeueReusableCellWithIdentifier:@"Charger plus"];
			if (cell == nil) 
			{		
//				cell = [[[UITableViewCell alloc] initWithFrame:CGRectZero reuseIdentifier:@"LoadMore"] autorelease];
                cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"Charger plus"];
...
//                }
				...
				cell.textLabel.font = [UIFont systemFontOfSize:16.0];
				cell.textLabel.textAlignment = UITextAlignmentCenter;
				cell.textLabel.text = NSLocalizedStringFromTable(@"Regarder les 10 résultats suivants", @"Localized", nil);
				cell.accessoryType = UITableViewCellAccessoryNone;
		...
		{
			cell = [tableView dequeueReusableCellWithIdentifier:NSLocalizedStringFromTable(@"Résultats", @"Localized", nil)];
			if (cell == nil) 
			{		
				cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:NSLocalizedStringFromTable(@"Résultats", @"Localized", nil)];
...
- (void)searchBar:(UISearchBar *)searchBar selectedScopeButtonIndexDidChange:(NSInteger)selectedScope
{
    ...
    switch (selectedScope)
    {
        ...
        default:
        {
            keywordSearchBar.placeholder = NSLocalizedStringFromTable(@"Rechercher RCSB dans la banque de protéine", @"Localized", nil);
        }; break;
    }
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error;
{
    ...
    if (currentSearchType == PROTEINDATABANKSEARCH)
    {
        connectionError = NSLocalizedStringFromTable(@"Impossible de se connecter à la banque de données de protéine", @"Localized", nil);
    }
    else
    {
        connectionError = NSLocalizedStringFromTable(@"Impossible de se connecter au PubChem", @"Localized", nil);
    }
    
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"Connection impossible", @"Localized", nil) message:connectionError
